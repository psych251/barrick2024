...
My planned sample size will be 100 participants. Given the discrepancy in the power analysis for Emotion Understanding (N=58 for 80% power and N=94 for 95% power) and for Loneliness (N=780 for 80% power and N=1290 for 95% power), I will be treating the first analysis (Emotion Prediction Accuracy ~ Emotion Understanding) as my main confirmatory hypothesis test. I will also measure loneliness, but I will treat the second analysis (Loneliness ~ Emotion Prediction Accuracy) as exploratory, given that the sample will be underpowered to detect the original effect.
...
"Loneliness was measured using the UCLA Loneliness Scale (Russell, 1996)" (Barrick et al. 2024).

### Procedure	

#### Participants and exclusions

The original study recruited participants from Amazon's Mechanical Turk, whereas I will recruit participants from Prolific. My replication will be conducted using Qualtrics, which is the same platform used in the original study. I will use the same inclusion criteria as the original study: "Study participation was restricted to workers in the United States with >95% approval ratings," as well as the same exclusion criteria: "Participants... were excluded if they completed less than half of the questions... or indicated an English comprehension less than 'Good'" (Barrick et al. 2024).

#### Tasks and Scales

After providing informed consent, participants will first complete the Emotion Transitions Task. Next, they will complete the Emotion Understanding Scale and the Loneliness Scale, with the order of these two scales randomized across participants. Finally, they will provide demographic information and receive a debrief.
...
For the UCLA Loneliness Scale and Toronto Alexithymia Scale, scores will be obtained by summing item ratings (with reverse-coding where applicable), as described in the scale instructions.
...
#### Loneliness: Exploratory

I am considering this analysis exploratory, given that my power analysis indicates that the replication will be underpowered to detect the original effect.

Analyses in the original study were as follows:

"We fit separate linear mixed-effects models for each social functioning measure using a restricted maximum likelihood estimation in R (REML; using the lmer() function of the lme4 package; Bates et al., 2015; R Core Team, 2021). Initial models consisted of social functioning score separately as the dependent variable (loneliness, social network size, perceived social support), a fixed effect of participants’ general emotion prediction accuracy scores, and a random intercept for study to account for study-level variation. p values and standardized coefficients were obtained using the parameters package in R (Lüdecke et al., 2020)." (Barrick et al. 2024).

Because I will not be collecting multiple waves of data, I will not need to run a multilevel model accounting for study-level variation. Instead, I will use the lm() function to conduct a simple linear regression with loneliness as the dependent variable and emotion prediction accuracy as the predictor. I will use the same method as the original study to obtain standardized coefficients from this model.
...
#### Compute loneliness and emotion understanding
...
ques_demo <- raw %>%
  ...
  mutate(
    lonely_total = rowSums(across(contains("lonely")))
  ) %>%
  select(
    sub_id, age, gender, race,
...
# ggsave(here("figures", "pilot", "pilot_predictions_plot.png"), width = 6, height = 6)
...
model_parameters(tas_acc_model, standardize = 'refit')
coef(tas_acc_model)
...
# ggsave(here("figures", "pilot", "tas_acc_plot.png"), width = 4, height = 3)
...
lonely_acc_model <- emo_predict_accuracy %>%
  left_join(ques_demo, by = c("sub_id" = "sub_id")) %>%
  lm(
    lonely_total ~ accuracy,
    data = .
  )

# Same method for standardizing coefficients as in original analyses

model_parameters(lonely_acc_model, standardize = 'refit')
coef(lonely_acc_model)
...
lonely_acc_model %>%
  predict_response(
    terms = c("accuracy [all]")
  ) %>%
  plot(show_data = TRUE) +
  labs(
    x = "Emotion Prediction Accuracy",
...
# ggsave(here("figures", "pilot", "lonely_acc_plot.png"), width = 4, height = 3)
